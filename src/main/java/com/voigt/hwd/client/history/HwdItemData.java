	...
	public static HwdItemRecord[] getNewRecords() {
		...
		for (User user : User.values()) {
			...
			if (userData != null) {
				...
				hwdItemRecords.add(hwdItemRecord);
			}
		}

		return hwdItemRecords.toArray(new HwdItemRecord[hwdItemRecords.size()]);
	}

	private HwdItemRecord[] dummy() {
		int cntTotalSeasons = 9;

		// 05.02.2000 (R�ckrunde Saison 1999/2000)
		Date hueniDate = new Date(2000 - 1900, 1, 5, 0, 0, 0);

		HwdItemRecord hueni = new HwdItemRecord(1, User.HUENI.toString(), hueniDate, cntTotalSeasons - 0, 1, 1, 2, 1,
